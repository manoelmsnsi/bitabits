<?php class Chamado extends Controller {   
    ...
    public function admin_listar(){
        ...
        if($acesso->acesso_valida("/".__CLASS__."/admin_listar/")==true){ 
            ...
            $inputs = $form->Listar("col-md-6", "Meus Chamados", "/".__CLASS__."/form/", "list",$chamado->listar_Chamado("INNER JOIN Status AS Prioridade ON Prioridade.id=Chamado.id_prioridade 
                ...
                INNER JOIN Pessoa AS Colaborador ON Colaborador.id=ChamadoGrupo.id_colaborador", NULL, "(Chamado.id_status<>'99' OR Chamado.id_status<>'100') AND ( Chamado.id_cliente='$id_cliente_servidor')", NULL, ' Chamado.id_status,Chamado.data_lancamento  ASC', "Chamado.id,Chamado.tipo AS Tipo,Cliente.nome AS Cliente,Colaborador.id AS id_colaborador,Colaborador.nome AS Colaborador,Chamado.titulo,Chamado.observacao AS 'Observação',Status.cor AS cor_Status,Status.Descricao AS Status,Prioridade.cor AS cor_Prioridade,Prioridade.descricao AS 'Prioridade',Prioridade.cor AS cor_prioridade",NULL,$pesquisa), "tabela1", array(array("acao"=>"/".__CLASS__."/form/","classe"=>"btn-sm btn-warning","icone"=>"edit"),array("acao"=>"/".__CLASS__."/visualizar/","classe"=>"btn-sm btn-rose","icone"=>"remove_red_eye"),array("acao"=>"/Logs/form/","classe"=>"btn-sm btn-danger","icone"=>"close")));
            ...
            ", NULL, "( Chamado.id_status='100')AND(Cliente.cpf='".$serverData[0]["cpf"]."') ", NULL, ' Chamado.data_lancamento  ASC', "Chamado.id,Chamado.tipo AS Tipo, Cliente.nome AS Cliente,Chamado.titulo,Chamado.observacao AS 'Observação',Status.cor AS cor_Status,Status.descricao AS 'Status',Prioridade.cor AS cor_Prioridade,Prioridade.descricao AS 'Prioridade'",NULL,$pesquisa), "tabela2", array(array("acao"=>"/ChamadoGrupo/form/","classe"=>"btn-sm btn-warning","icone"=>"supervised_user_circle"),array("acao"=>"/".__CLASS__."/visualizar/","classe"=>"btn-sm btn-rose","icone"=>"remove_red_eye"),array("acao"=>"/Logs/form/","classe"=>"btn-sm btn-danger","icone"=>"close")));
///AND(Cliente.cpf='".$serverData[0]["cpf"]."')//colocar no where para os clientes
  $form->card("Gerenciamento de " .__CLASS__, $inputs, "col-md-12", $comando, "POST", "list");
    ...
    //   $id=$this->getParams("id");  
            ...
            $inputs.= $form->Input("text", "titulo", "col-md-4", $chamado_dados["titulo"], $required,"Título");            
            ...
            $inputs.= $form->Input("text", "observacaos", "col-md-8", $chamado_dados["observacao"], $required,"Observação");            
        ...
        if($acesso->acesso_valida("/".__CLASS__."/admin_listar/")==true){ 
          ...
          //  $filiais=$acesso->acesso_filial(__CLASS__); 
           ...
           $menu = new MenuHelper("B", $Class, $AcaoForm, $MetodoDeEnvio);        
            ...
            $lista1= $form->Listar("col-md-12", $titulo, "/Chamado/form/id_chamado/$id/",$icone,$listar_chamado_filho,"tabela1",array(array("acao"=>"/Chamado/visualizar/","classe"=>"btn-sm btn-rose","icone"=>"remove_red_eye")));    
           ...
           $inputs.="<form class='col-md-12 row'>";
            ...
            $inputs.= $form->Abas($Tipo, "EBD", "col-md-12", 
                array(array("id" => "Chamados", "icone" => "contacts", "descricao" => "Chamados"), 
                array("id" => "Imagens", "icone" => "image", "descricao" => "Imagens")),
                array(array("id" => "Imagens", "dados" => "$lista2"),
                array("id" => "Chamados", "dados" => "$lista1","classe" => " active"))); 
          
            $form->card("Cadastrar Chamado",$inputs,"col-md-12","/Chamado/form/id/$id/","POST","donut_small");
        ...
        if ($acesso->acesso_valida($comando) == true) {
            ...
            $id = $chamado->cadastrar_chamado(
                    array(
                        ...
                        'id_cliente' => $info_servidor[0]["id_cliente"],
           ...
           $redirect->goToUrl('/Chamado/admin_listar/');
        } else {
            $this->view('error_permisao');
        }
    }
